document.addEventListener('DOMContentLoaded',()=>{
 ...
 const slides=document.querySelector('.slides');if(slides){const items=[...slides.children],dots=[...document.querySelectorAll('.dot')];let i=0;const go=n=>{i=(n+items.length)%items.length;slides.style.transform=`translateX(-${i*100}%)`;dots.forEach((d,j)=>d.classList.toggle('active',j===i))};document.querySelector('.next')?.addEventListener('click',()=>go(i+1));document.querySelector('.prev')?.addEventListener('click',()=>go(i-1));dots.forEach((d,j)=>d.addEventListener('click',()=>go(j)));setInterval(()=>go(i+1),6000)}
